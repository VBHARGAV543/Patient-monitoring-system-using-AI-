...
# ========== PHASE 4: HARDWARE TAMPERING ENDPOINT ==========

@app.post("/api/hardware/tamper")
async def tamper_hardware_vitals(request: Dict[str, Any]):
    """
    Apply controlled tampering to hardware patient's vitals for demonstration
    Reuses alarm_policy.apply_demo_tampering() function
    
    Request body:
    {
        "patient_id": int,
        "scenario": "NORMAL" | "MILD_DETERIORATION" | "CRITICAL_EMERGENCY" | "FALSE_POSITIVE"
    }
    """
    try:
        ...
        # Validate scenario
        valid_scenarios = ["NORMAL", "MILD_DETERIORATION", "CRITICAL_EMERGENCY", "FALSE_POSITIVE"]
        ...
        if patient['status'] != 'ACTIVE':
            raise HTTPException(status_code=400, detail="Can only tamper vitals for active patients")
        
        if not patient.get('hardware_mode', False):
            raise HTTPException(status_code=400, detail="Tampering only available for hardware mode patients")
        
        # Check if hardware vitals exist in buffer
        if patient_id not in hardware_vitals_buffer:
            raise HTTPException(status_code=404, detail="No hardware vitals found. Send sensor data first.")
        
        # Get current hardware vitals
        hw_data = hardware_vitals_buffer[patient_id]
        current_vitals = hw_data['vitals'].copy()
        
        # Apply tampering using alarm_policy function
        tampered_vitals = alarm_policy.apply_demo_tampering(
            vitals=current_vitals,
            scenario=scenario,
            patient_type=patient['patient_type']
        )
        
        # Update buffer with tampered vitals
        hw_data['vitals'] = tampered_vitals
        hw_data['timestamp'] = datetime.now(timezone.utc)
        hw_data['ttl'] = 30.0  # Reset TTL to 30 seconds
        
        # Broadcast tampered vitals through WebSocket
        await websocket_manager.broadcast_vitals({
            "patient_id": patient_id,
            "vitals": tampered_vitals,
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "source": "hardware_tampered",
            "scenario": scenario
        })
        
        logger.info(f"🎭 Applied {scenario} tampering to hardware patient {patient_id}")
        
        return {
            "status": "success",
            "message": f"Applied {scenario} tampering to hardware vitals",
            "vitals": tampered_vitals,
            "scenario": scenario,
            "ttl": 30.0
        }
        ...
        logger.error(f"Hardware tampering error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to tamper hardware vitals: {str(e)}")
