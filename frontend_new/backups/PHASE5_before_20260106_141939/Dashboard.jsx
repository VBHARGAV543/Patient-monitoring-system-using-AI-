...
import { getActivePatient, dischargePatient, checkBackendConnection, getHardwareVitals, tamperHardwareVitals } from '../services/api';
import {
  formatTimestamp,
  formatRelativeTime,
  checkVitalsStatus,
...
const Dashboard = () => {
  ...
  // Phase 3: Hardware vitals state
  const [hardwareVitals, setHardwareVitals] = useState(null);
  const [hardwareTTL, setHardwareTTL] = useState(null);
  const [hardwareStatus, setHardwareStatus] = useState('no_data');

  // Phase 4: Hardware tampering state
  const [showTamperDialog, setShowTamperDialog] = useState(false);
  ...
  // Phase 3: Poll hardware vitals for hardware mode patients
  useEffect(() => {
    if (!activePatient || !activePatient.hardware_mode) {
      setHardwareVitals(null);
      setHardwareStatus('no_data');
      return;
    }

    const pollHardwareVitals = async () => {
      try {
        const data = await getHardwareVitals(activePatient.id);
        
        if (data.status === 'success') {
          setHardwareVitals(data.vitals);
          setHardwareTTL(data.ttl_remaining);
          setHardwareStatus('success');
        } else if (data.status === 'expired') {
          setHardwareStatus('expired');
          setHardwareVitals(null);
        } else {
          setHardwareStatus('no_data');
          setHardwareVitals(null);
        ...
        console.error('Error fetching hardware vitals:', error);
    ...
    const interval = setInterval(pollHardwareVitals, 5000);
  ...
  // Phase 4: Handle hardware tampering
  const handleTamperVitals = async () => {
    if (!activePatient || !activePatient.hardware_mode) return;

    setTamperingInProgress(true);

    try {
      const result = await tamperHardwareVitals(activePatient.patient_id, selectedScenario);
      toast.success(`Applied ${selectedScenario} tampering to hardware vitals`);
      setShowTamperDialog(false);
      
      // Reset to NORMAL for next time
      setSelectedScenario('NORMAL');
    } catch (error) {
      console.error('Error tampering hardware vitals:', error);
      toast.error(error.response?.data?.detail || 'Failed to tamper hardware vitals');
    } finally {
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      <Container maxWidth="xl" sx={{ py: 3 }}>
        ...
        {/* Phase 3: Hardware Vitals Display */}
        {activePatient.hardware_mode && (
          <>
            ...
            {hardwareStatus === 'success' && hardwareVitals && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5 }}
              >
                <Paper elevation={3} sx={{ p: 3, mb: 3, bgcolor: 'secondary.light', borderLeft: 6, borderColor: 'secondary.main' }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <Sensors sx={{ fontSize: 32, color: 'secondary.main' }} />
                      <Typography variant="h6">Real Hardware Vitals</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                      <Chip 
                        label={`TTL: ${Math.floor(hardwareTTL)}s`} 
                        size="small" 
                        color={hardwareTTL > 15 ? 'success' : 'warning'}
                      />
                      <Chip label="ESP32 + MAX30100" size="small" variant="outlined" />
                    </Box>
                  </Box>
                  ...
                  <Grid container spacing={2}>
                    {hardwareVitals.HR && (
                      <Grid item xs={6} sm={4} md={2}>
                        <Box sx={{ textAlign: 'center' }}>
                          <Favorite sx={{ fontSize: 24, color: 'error.main' }} />
                          <Typography variant="h6">{hardwareVitals.HR}</Typography>
                    ...
                    {hardwareVitals.SpO2 && (
                      <Grid item xs={6} sm={4} md={2}>
                        <Box sx={{ textAlign: 'center' }}>
                          <Opacity sx={{ fontSize: 24, color: 'info.main' }} />
                          <Typography variant="h6">{hardwareVitals.SpO2}</Typography>
                    ...
                    {hardwareVitals.Temp && (
                      <Grid item xs={6} sm={4} md={2}>
                        <Box sx={{ textAlign: 'center' }}>
                          <Thermostat sx={{ fontSize: 24, color: 'warning.main' }} />
                          <Typography variant="h6">{hardwareVitals.Temp}</Typography>
                          <Typography variant="caption">°C</Typography>
                        </Box>
                      </Grid>
                    )}
                    {hardwareVitals.BP_sys && (
                      <Grid item xs={6} sm={4} md={2}>
                        <Box sx={{ textAlign: 'center' }}>
                          <MonitorHeart sx={{ fontSize: 24, color: 'success.main' }} />
                          <Typography variant="h6">{hardwareVitals.BP_sys}/{hardwareVitals.BP_dia || '--'}</Typography>
            ...
            {hardwareStatus === 'expired' && (
              <Alert severity="warning" sx={{ mb: 3 }}>
                Hardware vitals data expired. Waiting for new sensor readings...
              </Alert>
            )}

            {hardwareStatus === 'no_data' && (
              <Alert severity="info" sx={{ mb: 3 }}>
                No hardware vitals received yet. Make sure ESP32 sensor is active and sending data.
              </Alert>
            )}

            {hardwareStatus === 'error' && (
              <Alert severity="error" sx={{ mb: 3 }}>
                Failed to retrieve hardware vitals. Check backend connection.
              </Alert>
            )}

            {/* Phase 4: Tampering Control Button */}
            <Box sx={{ mt: 2, mb: 3, display: 'flex', justifyContent: 'center' }}>
              <Button
                variant="outlined"
                color="warning"
                onClick={() => setShowTamperDialog(true)}
                disabled={hardwareStatus !== 'success'}
                startIcon={<Warning />}
              >
                Apply Demonstration Tampering
              </Button>
            </Box>
          </>
        )}
      ...
      {/* Phase 4: Tampering Scenario Dialog */}
      <Dialog open={showTamperDialog} onClose={() => setShowTamperDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle>🎭 Hardware Vitals Tampering</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            Apply controlled tampering to demonstrate alarm scenarios. Changes will be applied to the current hardware vitals.
          </Typography>

          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Button
              variant={selectedScenario === 'NORMAL' ? 'contained' : 'outlined'}
              color="success"
            ...
            <Button
              variant={selectedScenario === 'MILD_DETERIORATION' ? 'contained' : 'outlined'}
              color="warning"
              onClick={() => setSelectedScenario('MILD_DETERIORATION')}
              fullWidth
            >
              MILD DETERIORATION - Slightly Abnormal
            </Button>

            <Button
              variant={selectedScenario === 'CRITICAL_EMERGENCY' ? 'contained' : 'outlined'}
              color="error"
              onClick={() => setSelectedScenario('CRITICAL_EMERGENCY')}
              fullWidth
            >
              CRITICAL EMERGENCY - Severely Abnormal
            </Button>

            <Button
              variant={selectedScenario === 'FALSE_POSITIVE' ? 'contained' : 'outlined'}
              color="info"
        ...
        <DialogActions>
          <Button onClick={() => setShowTamperDialog(false)} disabled={tamperingInProgress}>
            Cancel
          </Button>
          <Button 
            onClick={handleTamperVitals} 
            variant="contained" 
            color="warning"
            disabled={tamperingInProgress}
          >
            {tamperingInProgress ? <CircularProgress size={20} /> : 'Apply Tampering'}
          </Button>
        </DialogActions>
      </Dialog>
