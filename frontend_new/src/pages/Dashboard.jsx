...
import { getActivePatient, dischargePatient, checkBackendConnection, tamperHardwareVitals } from '../services/api';
import {
  formatTimestamp,
  formatRelativeTime,
  checkVitalsStatus,
...
const Dashboard = () => {
  ...
  const [tamperAnchorEl, setTamperAnchorEl] = useState(null);
  ...
  const { isConnected: wsConnected } = useDashboardWebSocket({
    onMessage: (data) => {
      ...
      if (data.event === 'patient_admitted') {
        ...
        // Legacy/direct sensor data update — backend broadcasts result object directly
        // Wrap it so updateFromSensorData can find data.result
        updateFromSensorData({ result: data });
      }
    },
    onConnect: () => {
  ...
  // Handle manual tampering
  const handleTamperVitals = async (scenario) => {
    if (!activePatient) return;
    
    setTampering(true);
    setTamperAnchorEl(null);
    
    try {
      await tamperHardwareVitals(activePatient.id, scenario);
      toast.success(`Tampered vitals: ${scenario}`, { icon: '🧪' });
    } catch (error) {
      console.error('Error tampering vitals:', error);
      toast.error('Failed to tamper vitals');
    } finally {
      setTampering(false);
    }
  };
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      <Container maxWidth="xl" sx={{ py: 3 }}>
        ...
        <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              {activePatient.hardware_mode && (
                <Button
                  ...
                  startIcon={<Science />}
                  endIcon={<ExpandMore />}
                  onClick={(e) => setTamperAnchorEl(e.currentTarget)}
                  disabled={tampering}
                >
                  {tampering ? 'Tampering...' : 'Tamper Vitals'}
                </Button>
              )}
              <Button
                variant="outlined"
          ...
          {/* Tamper Menu */}
          <Menu
            anchorEl={tamperAnchorEl}
            open={Boolean(tamperAnchorEl)}
            onClose={() => setTamperAnchorEl(null)}
          >
            <MenuItem onClick={() => handleTamperVitals('NORMAL')}>
              <ListItemIcon><Sensors fontSize="small" /></ListItemIcon>
              <ListItemText>Normal Vitals</ListItemText>
            </MenuItem>
            <MenuItem onClick={() => handleTamperVitals('HIGH_HR')}>
              <ListItemIcon><Favorite fontSize="small" color="warning" /></ListItemIcon>
              <ListItemText>High Heart Rate</ListItemText>
            </MenuItem>
            <MenuItem onClick={() => handleTamperVitals('LOW_SPO2')}>
              <ListItemIcon><Opacity fontSize="small" color="warning" /></ListItemIcon>
              <ListItemText>Low SpO2</ListItemText>
            </MenuItem>
            <MenuItem onClick={() => handleTamperVitals('HIGH_TEMP')}>
              <ListItemIcon><Thermostat fontSize="small" color="error" /></ListItemIcon>
              <ListItemText>High Temperature</ListItemText>
            </MenuItem>
            <MenuItem onClick={() => handleTamperVitals('CRITICAL')}>
              <ListItemIcon><Warning fontSize="small" color="error" /></ListItemIcon>
              <ListItemText>Critical Emergency</ListItemText>
            </MenuItem>
          </Menu>
        </Paper>
