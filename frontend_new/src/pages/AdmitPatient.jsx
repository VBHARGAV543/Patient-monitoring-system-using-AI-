...
const patientSchema = z.object({
  ...
  hardware_mode: z.boolean().optional(),
  ...
  // Critical ward staff
  doctor_primary: z.string().optional(),
  nurse_assigned: z.string().optional(),
  doctor_assistant: z.string().optional(),
}).superRefine((data, ctx) => {
  if (data.patient_type === 'CRITICAL') {
    if (!data.doctor_primary || data.doctor_primary.trim() === '') {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Primary doctor is required for Critical Ward', path: ['doctor_primary'] });
    }
    if (!data.nurse_assigned || data.nurse_assigned.trim() === '') {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Assigned nurse is required for Critical Ward', path: ['nurse_assigned'] });
    }
  }
});
  ...
  return (
    ...
    >
      <Box sx={{ width: '100%', maxWidth: '650px', mx: 'auto' }}>
        ...
        >
          <Paper elevation={10} sx={{ p: { xs: 3, sm: 4, md: 5 }, borderRadius: 4 }}>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                      ...
                      label={
                        <Box>
                          <Typography variant="body1">
                            Hardware Mode (Real Sensor Data)
                          </Typography>
                          <Typography variant="caption" color="text.secondary">
                            Use real vitals from hardware sensors instead of simulation
                          </Typography>
                        </Box>
                      }
                    />
              ...
              {/* ── Critical Ward Staff Assignment ── */}
                ...
                >
                  ...
                  >
                    ...
                    <Typography variant="caption" color="text.secondary" sx={{ mb: 2, display: 'block' }}>
                      Assign medical staff responsible for this critical patient.
                    </Typography>
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <TextField
                            ...
                            fullWidth
                            required
                            error={!!errors.doctor_primary}
                            helperText={errors.doctor_primary?.message}
                            disabled={submitting || !bandStatus.available}
                            InputProps={{ startAdornment: <span style={{ marginRight: 8 }}>👨‍⚕️</span> }}
                          />
                        )}
                      />
                      {/* Assigned Nurse — required */}
                      <Controller
                        name="nurse_assigned"
                        control={control}
                        render={({ field }) => (
                          <TextField
                            ...
                            placeholder="e.g. Nurse Priya Singh"
                            fullWidth
                            required
                            error={!!errors.nurse_assigned}
                            helperText={errors.nurse_assigned?.message}
                            disabled={submitting || !bandStatus.available}
                            InputProps={{ startAdornment: <span style={{ marginRight: 8 }}>👩‍⚕️</span> }}
                          />
                        )}
                      />
                      {/* Assistant Doctor — optional */}
                      <Controller
