...
export const useWebSocket = (path, options = {}) => {
  ...
  // Stable refs for callbacks — updated every render so the latest handler is always used,
  // but they don't cause connect() to be recreated.
  ...
  const connect = useCallback(() => {
    ...
    attemptConnection();
  // Only path and clearReconnectTimeout are stable — callbacks use refs so they don't need to be deps
  }, [path, clearReconnectTimeout]); // eslint-disable-line react-hooks/exhaustive-deps
