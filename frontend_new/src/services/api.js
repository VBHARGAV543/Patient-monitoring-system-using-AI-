...
/**
 * Manually tamper hardware vitals for testing
 * @param {number} patientId - Patient ID
 * @param {string} scenario - NORMAL, HIGH_HR, LOW_SPO2, HIGH_TEMP, CRITICAL
 * @returns {Promise} Tampered vitals result
 */
export const tamperHardwareVitals = async (patientId, scenario) => {
  const response = await apiClient.post(`/api/hardware/tamper/${patientId}`, null, {
    params: { scenario }
  });
  return response.data;
};
