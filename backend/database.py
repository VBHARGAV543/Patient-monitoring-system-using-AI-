    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS patients (
                ...
                hardware_mode BOOLEAN DEFAULT FALSE,
                disease VARCHAR(255),
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            INSERT INTO patients (
                name, age, problem, patient_type, demo_mode, demo_scenario, hardware_mode,
            ...
            RETURNING id, name, age, gender, blood_type, weight, height, problem, 
                      ...
                      emergency_phone, patient_type, status, demo_mode, demo_scenario, hardware_mode,
                      disease, body_strength, genetic_condition, admission_time, discharge_time
        """, name, age, problem, patient_type, demo_mode, demo_scenario, hardware_mode,
...
async def get_patient_by_id(patient_id: int) -> Optional[Dict[str, Any]]:
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            SELECT p.id, p.name, p.age, p.gender, p.blood_type, p.weight, p.height, p.problem, 
                   p.medical_history, p.allergies, p.current_medications, p.emergency_contact, 
                   p.emergency_phone, p.patient_type, p.status, p.demo_mode, p.demo_scenario, p.hardware_mode,
                   p.disease, p.body_strength, p.genetic_condition,
                   p.admission_time, p.discharge_time,
                   ba.band_id, ba.assigned_at
            FROM patients p
            LEFT JOIN band_assignment ba ON p.id = ba.patient_id AND ba.released_at IS NULL
            WHERE p.id = $1
        """, patient_id)
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            SELECT p.id, p.name, p.age, p.gender, p.blood_type, p.weight, p.height, 
                   ...
                   p.demo_mode, p.demo_scenario, p.hardware_mode, p.disease, p.body_strength, p.genetic_condition,
                   p.admission_time, p.discharge_time,
                   ba.band_id, ba.assigned_at
            FROM patients p
...
async def auto_discharge_on_restart() -> int:
    """On server restart, discharge any patients left ACTIVE from a previous session
    and release any open band assignments. Returns number of patients cleaned up."""
    async with pool.acquire() as conn:
        # Release all open band assignments first
        await conn.execute("""
            UPDATE band_assignment
            SET released_at = NOW()
            WHERE released_at IS NULL
        """)
        # Mark any still-ACTIVE patients as DISCHARGED
        result = await conn.execute("""
            UPDATE patients
            SET status = 'DISCHARGED', discharge_time = NOW()
            WHERE status = 'ACTIVE'
...
async def is_band_available(band_id: str = None) -> bool:
    ...
    Auto-heals stale assignments where the patient is no longer ACTIVE."""
    if band_id is None:
        band_id = BAND_ID
    
    async with pool.acquire() as conn:
        # Auto-release stale assignments: band is 'occupied' but patient is discharged or missing
        await conn.execute("""
            UPDATE band_assignment ba
            SET released_at = NOW()
            WHERE ba.band_id = $1
              AND ba.released_at IS NULL
              AND NOT EXISTS (
                  SELECT 1 FROM patients p
                  WHERE p.id = ba.patient_id AND p.status = 'ACTIVE'
              )
        """, band_id)
...
async def force_release_band(band_id: str = None) -> int:
    """Force-release all active assignments for this band. Returns count released."""
    if band_id is None:
        band_id = BAND_ID
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            SELECT p.id, p.name, p.age, p.gender, p.blood_type, p.weight, p.height,
                   p.problem, p.medical_history, p.allergies, p.current_medications,
                   p.emergency_contact, p.emergency_phone, p.patient_type, p.status,
                   p.demo_mode, p.demo_scenario, p.hardware_mode, p.disease,
                   p.body_strength, p.genetic_condition,
                   p.admission_time, p.discharge_time,
                   ba.band_id, ba.assigned_at
            FROM patients p
            JOIN band_assignment ba ON p.id = ba.patient_id
...
async def get_admitted_patients(patient_type: Optional[str] = None) -> List[Dict[str, Any]]:
    """Get all currently admitted (ACTIVE) patients, optionally filtered by patient_type"""
    async with pool.acquire() as conn:
        if patient_type:
            rows = await conn.fetch("""
                SELECT id, name, age, problem, patient_type, status, admission_time
                FROM patients
                WHERE status = 'ACTIVE' AND patient_type = $1
